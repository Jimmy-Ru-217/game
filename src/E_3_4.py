...
class KO(Template):
    ...
    def detach(self):
        ...
        if self.is_found(self.ps.battle) and not self.is_found(self.ps.restore):
            ...
            logging.info("队伍重创")
            return 18
        elif self.is_found(self.ps.e3_4) and not self.repair_flag and not self.is_found(self.ps.normal):
            logging.info("任务选择")
            return 2
        elif self.is_found(self.ps.normal) :
            logging.info("选择作战")
            return 3
        elif  self.is_found(self.ps.boss3) and self.is_found(self.ps.start) :
            print('start')
            logging.info("初始任务地图并且执行")
            return 4
        elif self.is_found(self.ps.end3_4_1) and self.is_found(self.ps.end3_4_2):
            logging.info("结束本局")
            return 5
            ...
            logging.info("获得核心")
            return 7
        elif self.is_found(self.ps.e3_4) and self.repair_flag:
